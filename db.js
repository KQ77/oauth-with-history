...
process.env.GITHUB_CLIENT_ID = require(`./secrets.js`).client_id;
process.env.GITHUB_CLIENT_SECRET = require('./secrets.js').client_secret;
if (process.env.LOGGING) {
  delete config.logging;
}
const conn = new Sequelize(
  process.env.DATABASE_URL || 'postgres://localhost/acme_db_with_history',
  config
);
...
const getUserFromGithubUser = async ({ login, id, avatar_url, ...other }) => {
...
User.authenticate = async (code) => {
  ...
  // possibly here is where we should update the UserLogin database to increment the #logins
  await UserLogin.create({ userId: user.id });
